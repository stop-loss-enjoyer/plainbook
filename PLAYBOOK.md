...
before a trade is opened, and how the position is held after. It is not a
plan for a day and not a note; it is the system itself, and it changes by
version, not by the week. A journal can hold several, one per way of
...
draft and change freely. Once a
trade has been through the checklist under the number, a change of the
rules asks for a new number, and the rules as they were are kept under
*Earlier versions* on the page: a trade keeps showing the rules it was
ticked against, whatever the playbook says later. Only the rules and the
names of the setups are held to this; the intro, the notes, the limits and
the status change freely.
...
at the top of the page. Not a rule and not ticked; the place for what the
...
**A rule** is two parts. The few words are what the checklist shows in the
trade form: short enough to be read, not skimmed, at the moment of
opening. The whole rule is the explanation behind them, shown on the page
...
Limits the journal can count are not filters: the count of trades this week,
the loss of the week, the positions open at once go under *Limits* and are
shown by the journal itself above the checklist, so they need no box.
...
the trade is closed, not when it is opened, in the form that closes it and in
the form of a closed trade. While the trade is open they stand on its page
as a plain list, to be kept in mind. Three to five is a list that gets
ticked; ten is one that gets skipped.
...
against its cap.
A figure turns red where this trade would go past the limit. Nothing is
refused; the figure is there to be seen before the boxes are ticked.
...
Not in the form: the page of the playbook ends with *Review*, one field and
a dated entry each time, with screenshots pasted under it. That is where a
...
number of boxes ticked; nothing is refused. The rules left unticked are
recorded with the trade, and under every empty box stands a line for why:
the fact, not the verdict. Leaving every box empty is recorded too, as every rule not
met.
...
keep their own pages. The table is the answer to what a rule is worth, in R
...
and its version but no checklist: nobody ticked the rules for them, they
say nothing about any rule, and they do not hold the version. Open each one
and tick the rules from memory if it is worth it.
